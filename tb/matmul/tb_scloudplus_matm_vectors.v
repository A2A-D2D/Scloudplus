...
module tb_scloudplus_matm_vectors;
    ...
    reg [REQ_BITS-1:0] req_mem [0:31];
    reg [BLOCK_BITS-1:0] exp_mem [0:31];
    integer errors;
    integer req_idx;
    integer exp_idx;
    integer wait_count;
    ...
    initial begin
        ...
        blk_in_valid = 1'b0;
        c_block_ready = 1'b1;
        cfg_b_active = B[CFG_WIDTH-1:0];
        cfg_q_active = Q_WIDTH[CFG_WIDTH-1:0];
        cfg_coeff_mode = 2'd0;
        ...
        run_case_keygen_as;
        run_case_enc_c1_transpose;
        run_case_dec_c1s;
        run_case_c_keygen_as;
        run_case_c_enc_c1_transpose;
        run_case_c_dec_c1s;

        if (errors == 0) begin
            $display("TB_PASS scloudplus_matm_vectors");
        end else begin
            $display("TB_FAIL scloudplus_matm_vectors errors=%0d", errors);
        end
        $finish;
    end
            ...
            while (!done) begin
                ...
                if (c_block_valid) begin
                    if (c_block !== exp_mem[exp_idx]) begin
                        $display("FAIL %0s output=%0d got=%h exp=%h row=%0d col=%0d", name, exp_idx, c_block, exp_mem[exp_idx], c_row_blk, c_col_blk);
                        errors = errors + 1;
                    end
                    exp_idx = exp_idx + 1;
                end
    ...
    initial begin
        #100000;
        $display("TB_TIMEOUT scloudplus_matm_vectors");
        $finish;
    end
