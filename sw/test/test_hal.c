/*
 * test_hal.c — Comprehensive test for SCLOUD+ HAL (SW backend).
 *
 * Tests:
 *   1. Block-level matrix multiply against known vectors
 *   2. MsgEncode/MsgDecode roundtrip (zero noise)
 *   3. MsgEncode/MsgDecode roundtrip (small noise)
 *   4. End-to-end KeyGen → Encaps → Decaps flow
 */
...
/* =========================================================================
 * Test 1: Block-level matmul against zero/identity
 * ========================================================================= */
...
static void test_matmul_block(void) {
    ...
    /* A = identity, S = all ones → C should equal A */
    memset(a_block, 0, sizeof(a_block));
    memset(s_block, 0, sizeof(s_block));
    for (int i = 0; i < B; i++) {
        a_block[i * B + i] = 100;  /* diagonal = 100 */
        /* S[i][i] = +1 (01 in packed format) */
        int bit = (i * B + i) * 2;
...
static void test_msgfunc_roundtrip_tau3(void) {
    ...
    int tau = 3;
    int msg_bytes = 8;            /* tau=3: 8 bytes per block */
    ...
    /* Test patterns */
    const char *patterns[] = {
        "\x00\x00\x00\x00\x00\x00\x00\x00",  /* all zero */
        "\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF",  /* all ones */
        "\x12\x34\x56\x78\x9A\xBC\xDE\xF0",  /* sample */
        "\xA5\x5A\xA5\x5A\xA5\x5A\xA5\x5A",  /* alternating */
    };
    ...
    for (int p = 0; p < n_pat; p++) {
        ...
        if (memcmp(msg, decoded, 8) != 0) {
            printf("FAIL at pattern %d: msg=%02x%02x... dec=%02x%02x...\n",
                   p, msg[0], msg[1], decoded[0], decoded[1]);
            ok = 0;
            break;
        }
    }
...
static void test_msgfunc_roundtrip_tau4(void) {
    ...
    int tau = 4;
    int msg_bytes = 12;  /* tau=4: 12 bytes per block */
...
static void test_msgfunc_multiblock(void) {
    ...
    int mu_bytes = para.mu / 8;     /* 8 */
    int mu_conut = para.muConut;     /* 2 */
    int total = mu_bytes * mu_conut; /* 16 */
    int block_size = SCLOUD_BW_REAL;

    uint8_t msg[16];
    uint16_t matrixM[64];  /* 2 blocks * 32 */
    uint8_t decoded[16];
...
static void test_bdd_noise_resilience(void) {
    ...
    /* Add small noise: ±2 per coordinate, alternating */
    for (int i = 0; i < 32; i++) {
        int n = (i & 1) ? -2 : 2;
    ...
    /* Small noise should still decode correctly */
    int ok = (memcmp(msg, decoded, 8) == 0);
    if (ok) PASS(); else FAIL("noise caused decode error");
}

/* =========================================================================
 * Test 6: End-to-end MsgEncode/MsgDecode within KEM context
 * (ss=16, zero noise, no compression — tests the encode/decode pipeline)
 * ========================================================================= */
...
static void test_kem_msg_roundtrip(void) {
    ...
    /* Decode (zero noise — should be perfect) */
    hal_msgdecode(matrixM, &para, decoded);
 ...
 * Tests that all KEM functions run without crashing and produce valid output.
 * NOTE: Full roundtrip shared secret match requires the FO transform
 * with re-encryption verification, which is a future enhancement.
 * ========================================================================= */
...
/* =========================================================================
 * Test 7: MatMul AS_E small-scale correctness check
 * ========================================================================= */

static void test_matmul_as_e(void) {
    TEST("MatMul AS_E small-scale (2×3 * 3×2)");
    /* Manual small test: m=2, n=3, nbar=2 */
    uint8_t seedA[16] = {0};
    ScloudPlusPara para;
    ...
    int16_t S[6] = {1, 0, -1, 0, 1, 0};  /* 3×2, row-major: [[1,0],[-1,0],[0,1]] */
    ...
    /* B should be nonzero (A is generated from seed, S is nonzero) */
    int nonzero = 0;
...
int main(void) {
    printf("=== SCLOUD+ HAL SW Backend Test Suite ===\n\n");
