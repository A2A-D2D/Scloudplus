...
/* =========================================================================
 * AES-128-ECB based A matrix generation (simplified for SW model)
 *
 * In real openHiTLS, A is generated by encrypting counter values with
 * AES-128-ECB. For the functional SW model, we use a deterministic
 * PRNG seeded from seedA to produce reproducible pseudo-random Q-domain
 * values. This is NOT cryptographically correct but produces the same
 * shaped output for functional testing.
 *
 * TODO: Replace with real AES-128-ECB when linking against OpenSSL.
 * ========================================================================= */

/* Simple LCG for deterministic expansion (placeholder for AES) */
static uint32_t lcg_state;

static void lcg_seed(const uint8_t *seed, int len) {
    lcg_state = 0;
    for (int i = 0; i < len; i++) {
        lcg_state = lcg_state * 1103515245u + seed[i] + 12345u;
    }
    if (lcg_state == 0) lcg_state = 1;
}

static uint16_t lcg_rand12(void) {
    lcg_state = lcg_state * 1103515245u + 12345u;
    return (uint16_t)((lcg_state >> 16) & SCLOUD_MOD_Q);
}

static void generate_a_rows(const uint8_t *seedA, int row_start, int n_rows,
                            int n_cols, uint16_t *a_out) {
    /* Deterministic expansion per row */
    for (int i = 0; i < n_rows; i++) {
        /* Seed per row = seedA + row index */
        uint8_t row_seed[20];
        memcpy(row_seed, seedA, 16);
        row_seed[16] = (row_start + i) & 0xFF;
        row_seed[17] = ((row_start + i) >> 8) & 0xFF;
        row_seed[18] = 0;
        row_seed[19] = 0;
        lcg_seed(row_seed, 20);
        for (int j = 0; j < n_cols; j++) {
            a_out[i * n_cols + j] = lcg_rand12();
        }
    }
}
...
int hal_sw_matmul_as_e(const uint8_t *seedA, const int16_t *S,
    ...
    for (int rb = 0; rb < row_blocks; rb++) {
        ...
        if (!a_batch) return -1;
        generate_a_rows(seedA, r_start, n_rows_batch, n, a_batch);
...
int hal_sw_matmul_sa_e(const uint8_t *seedA, const int16_t *S,
    ...
    if (!A_full) return -1;
    generate_a_rows(seedA, 0, m, n, A_full);
    ...
    for (int j = 0; j < mbar; j++) {
        for (int i = 0; i < n; i++) {
            ...
            for (int k = 0; k < m; k++) {
                int16_t sv = S[j * m + k];
                sum += (int32_t)sv * A_full[k * n + i];
            }
            C[j * n + i] = (C[j * n + i] + (uint16_t)(sum & SCLOUD_MOD_Q)) & SCLOUD_MOD_Q;
        }
    }
...
void hal_sw_matmul_cs(const uint16_t *C1, const int16_t *S,
    ...
    /* out[i][j] = sum_k C1[i][k] * S[j][k] */
    for (int i = 0; i < mbar; i++) {
        for (int j = 0; j < nbar; j++) {
            int32_t sum = 0;
            for (int k = 0; k < n; k++) {
                int16_t sv = S[j * n + k];
                sum += (int32_t)C1[i * n + k] * sv;
            }
            out[i * nbar + j] = (uint16_t)(sum & SCLOUD_MOD_Q);
        }
    }
}
