 ...
 *
 * Deterministic LCG-based expansion per row from seedA.
 * TODO: Replace with real AES-128-ECB for production.
 * ========================================================================= */
...
void sw_generate_a_rows(const uint8_t *seedA, int row_start, int n_rows,
    ...
    for (int i = 0; i < n_rows; i++) {
        ...
        for (int s = 0; s < 20; s++) {
            lcg_state = lcg_state * 1103515245u + row_seed[s] + 12345u;
        }
        if (lcg_state == 0) lcg_state = 1;

        for (int j = 0; j < n_cols; j++) {
            lcg_state = lcg_state * 1103515245u + 12345u;
            a_out[i * n_cols + j] = (uint16_t)((lcg_state >> 16) & SCLOUD_MOD_Q);
        }
    }
}
...
int sw_sample_psi(const uint8_t *seed, const ScloudPlusPara *para, int16_t *S) {
    ...
    /* S is n × nbar (row-major): row pos, column col */
    for (int col = 0; col < nbar; col++) {
        ...
        while (count < h1 * 2 && attempts < n * 10) {
            ...
            if (S[pos * nbar + col] == 0) {
                S[pos * nbar + col] = (int16_t)(1 - 2 * (count & 1));
                count++;
            }
            attempts++;
        }
...
int sw_sample_phi(const uint8_t *seed, const ScloudPlusPara *para, int16_t *S_prime) {
    ...
    /* S_prime is mbar × m (row-major, stride=m): row col, column pos.
     * Matches matmul convention S[col * m + pos] = row=col, col=pos, stride=m. */
