...
int scloudplus_keygen(uint8_t *pk, uint16_t *pk_len,
    ...
    /* 5. Pack keys */
    *pk_len = (uint16_t)((m * nbar * 3 + 1) / 2);
    *sk_len = (uint16_t)((n * nbar + 3) / 4);
    sw_pack_pk(B, para, pk);
    sw_pack_sk(S, para, sk);

    /* Include seedA in public key after B */
    memcpy(pk + *pk_len, seedA, SCLOUD_SEED_A_LEN);
    *pk_len += SCLOUD_SEED_A_LEN;
