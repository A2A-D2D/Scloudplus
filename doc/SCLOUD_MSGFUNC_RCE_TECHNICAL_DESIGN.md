...
BDD8、BDD16、BDD32 因此各增加一个显式首次子调用 launch 状态：父节点先在本层锁存事务，下一拍再由本层状态启动子节点，从结构上切断跨三级 start/CE 控制链。新增编码仍使用原有 4-bit 状态寄存器，不恢复已删除的宽目标或候选缓存；一次 BDD32 调用固定增加 21 拍，不改变算术、严格 `<` 平局规则、40 DSP 架构或外部握手。纯 Verilog 检查、BDD4 100 组、距离 200 随机加 2 平局及完整 RCE 回归均通过，时序改善需由下一轮约束综合确认。
